...
// Same border/bg/text triple as RUN_STATUS_STYLES above, so a verdict pill
// reads with the same weight as a run-status pill.
...
const REVIEW_READINESS_LABELS: Record<ReviewReadiness, string> = {
  ready_for_review: 'Ready for review',
  needs_rewrite: 'Needs rewrite',
};

const MODEL_STATUS_LABELS: Record<EvaluationModelStatus, string> = {
  applied: 'Model applied',
  unavailable: 'Model unavailable',
  skipped: 'Model skipped',
};

const EVALUATION_DIMENSION_LABELS: Record<(typeof EVALUATION_DIMENSIONS)[number], string> = {
  coherence: 'Coherence',
  ageFit: 'Age fit',
 ...
 */
function EvaluationEntry({ evaluation }: { evaluation: StoredEvaluation }) {
  return (
 ...
 *
 * Unit 7c adds a third detail panel, Evaluation, spanning both columns below
 * the timeline and checkpoint. It matters for the same kind of reason the
 * checkpoint view does: agent_evaluations (migration 108) is the only record
 * of *why* a run's grade came out the way it did, and evaluation.shared.ts's
 * whole design point is that the model never gets a vote on verdict or
 * review readiness -- only the deterministic layer does. Without this panel
 * that split is invisible from the admin surface; an operator would see a
 * run reach "Evaluated" and have to trust it, rather than being able to see
 * which specific deterministic check fired and which concerns were merely
 * advisory. It also has to survive three distinct states, same discipline as
 * the run-list empty states above: a run that hasn't reached "Evaluated" yet
 * reads differently from one that reached it before migration 108 existed
 * (or whose write failed) -- collapsing those two into one blank panel would
 * misreport "not evaluated" for a run that, in fact, was.
 */
                              ...
                              {loadingDetailId === run.id ? (
                                ...
                                <div className="grid gap-5 lg:grid-cols-2">
                                  ...
                                  <div className="lg:col-span-2">
                                    ...
                                    {detail && detail.evaluations.length > 0 ? (
                                      ...
                                      <p className="text-sm text-neutral-500">
                                        This run passed the {stageLabel('evaluated')} stage without storing a grade —
                                        either it ran before migration 108 was applied, or the write failed. The stage
                                        timeline on the left records what was computed.
                                      </p>
                                    ) : (
                                      <p className="text-sm text-neutral-500">
                                        Not evaluated yet — this run has not reached the {stageLabel('evaluated')}{' '}
                                        stage.
                                      </p>
                                    )}
