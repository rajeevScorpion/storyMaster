...
import { Clapperboard, CloudRain, Loader2, Move, Pause, Play, Sun, Trash2, X } from 'lucide-react';
...
function RangeField({
  ...
  return (
    <label className="block rounded-xl border border-white/8 bg-white/[0.025] px-3 py-2.5">
      ...
      <input className="mt-2 h-2 w-full cursor-pointer accent-emerald-400" type="range" min={min} max={max} step={step} value={value} onChange={(event) => onChange(Number(event.target.value))} />
    </label>
  );
}
