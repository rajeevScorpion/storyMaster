      ...
      <section role="dialog" aria-modal="true" aria-labelledby="story-effects-title" className="flex max-h-[96dvh] w-full max-w-6xl flex-col overflow-hidden rounded-[28px] border border-white/10 bg-neutral-950 shadow-2xl">
        ...
        <div className="grid min-h-0 flex-1 overflow-y-auto lg:grid-cols-[minmax(320px,0.9fr)_minmax(360px,1.1fr)]">
          ...
          <div className="space-y-4 p-5">
            ...
            <section className="space-y-3 rounded-2xl border border-white/10 bg-white/[0.025] p-4"><SectionToggle label="Particles" enabled={draft.particles.enabled} onChange={(enabled) => update({ ...draft, particles: { ...draft.particles, enabled } })} /><select value={draft.particles.type} onChange={(event) => update({ ...draft, particles: { ...draft.particles, type: event.target.value as StoryEffectConfig['particles']['type'] } })} className="w-full rounded-xl border border-white/10 bg-neutral-900 px-3 py-2 text-sm text-white"><option value="dust">Dust</option><option value="snow">Snow</option><option value="rain">Rain</option></select><div className="grid gap-2 sm:grid-cols-2"><RangeField label="Amount" value={draft.particles.amount} min={0} max={120} step={1} onChange={(amount) => update({ ...draft, particles: { ...draft.particles, amount } })} /><RangeField label="Density" value={draft.particles.density} min={0} max={100} step={1} suffix="%" onChange={(density) => update({ ...draft, particles: { ...draft.particles, density } })} /><RangeField label="Opacity" value={draft.particles.opacity} min={0} max={1} step={0.05} onChange={(opacity) => update({ ...draft, particles: { ...draft.particles, opacity } })} /><RangeField label="Speed" value={draft.particles.speed} min={0} max={3} step={0.05} onChange={(speed) => update({ ...draft, particles: { ...draft.particles, speed } })} /><RangeField label="Size" value={draft.particles.size} min={0.25} max={3} step={0.05} onChange={(size) => update({ ...draft, particles: { ...draft.particles, size } })} /><RangeField label="Direction" value={draft.particles.direction} min={-180} max={180} step={1} suffix="°" onChange={(direction) => update({ ...draft, particles: { ...draft.particles, direction } })} /></div><input type="color" value={draft.particles.color} onChange={(event) => update({ ...draft, particles: { ...draft.particles, color: event.target.value } })} className="h-9 w-full rounded-lg bg-transparent" aria-label="Particle color" /></section>

            <section className="space-y-3 rounded-2xl border border-white/10 bg-white/[0.025] p-4"><SectionToggle label="Atmosphere" enabled={draft.atmosphere.enabled} onChange={(enabled) => update({ ...draft, atmosphere: { ...draft.atmosphere, enabled } })} /><select value={draft.atmosphere.type} onChange={(event) => update({ ...draft, atmosphere: { ...draft.atmosphere, type: event.target.value as StoryEffectConfig['atmosphere']['type'] } })} className="w-full rounded-xl border border-white/10 bg-neutral-900 px-3 py-2 text-sm text-white"><option value="glow">Glow</option><option value="mist">Mist</option></select><div className="grid gap-2 sm:grid-cols-2"><RangeField label="Opacity" value={draft.atmosphere.opacity} min={0} max={1} step={0.05} onChange={(opacity) => update({ ...draft, atmosphere: { ...draft.atmosphere, opacity } })} /><RangeField label="Speed" value={draft.atmosphere.speed} min={0} max={3} step={0.05} onChange={(speed) => update({ ...draft, atmosphere: { ...draft.atmosphere, speed } })} /><RangeField label="Scale" value={draft.atmosphere.scale} min={0.5} max={3} step={0.05} onChange={(scale) => update({ ...draft, atmosphere: { ...draft.atmosphere, scale } })} /><RangeField label="Intensity" value={draft.atmosphere.intensity} min={0} max={100} step={1} suffix="%" onChange={(intensity) => update({ ...draft, atmosphere: { ...draft.atmosphere, intensity } })} /></div></section>
          </div>
        </div>
