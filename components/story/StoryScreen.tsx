...
import { ArrowRight, RefreshCcw, BookOpen, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function StoryScreen() {
  ...
  if (!session || session.beats.length === 0) return null;
                  ...
                  >
                    {currentBeat.options.map((option, index) => (
                      <motion.button
                        ...
                        disabled={isLoading}
                        className="w-full text-left group bg-neutral-900/60 hover:bg-neutral-800 backdrop-blur-md border border-white/5 hover:border-white/20 rounded-2xl p-6 transition-all duration-300 flex items-center justify-between"
                      >
                        <div>
