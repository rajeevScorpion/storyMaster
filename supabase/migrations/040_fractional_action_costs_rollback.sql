-- 040_fractional_action_costs_rollback.sql
-- Restore whole-beat wallet accounting. Fractional values are rounded up so balances
-- and previously charged action costs do not understate existing obligations.
...
ALTER TABLE public.pricing_action_costs
  ALTER COLUMN beat_cost TYPE integer USING CEIL(beat_cost)::integer;

ALTER TABLE public.beat_grants
  ALTER COLUMN beats_total TYPE integer USING CEIL(beats_total)::integer,
  ALTER COLUMN beats_remaining TYPE integer USING CEIL(beats_remaining)::integer;

ALTER TABLE public.beat_usage_events
  ALTER COLUMN beat_cost TYPE integer USING CEIL(beat_cost)::integer;

ALTER TABLE public.beat_spend_reservations
  ALTER COLUMN requested_beat_cost TYPE integer USING CEIL(requested_beat_cost)::integer;

ALTER TABLE public.beat_usage_allocations
  ALTER COLUMN beats_consumed TYPE integer USING CEIL(beats_consumed)::integer;
