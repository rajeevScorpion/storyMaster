...
are confirmed live (see the migration table row and "Shared branching" below). **Production now has all of them except 115** (applied by hand 2026-09-16, verified against the prod ledger).
The code promoted to `main` the same day as `084c2ed` and deployed. 115 is the single deliberate holdout —
see its row below.
...
only gap. The signed-out gallery was verified populated after 116, and a prompt-only beat generated cleanly
on a real account.

**Storyline sign-in return promoted 2026-09-16** (`main` at `897a205`, no migrations; plan
`docs/storyline-signin-return-plan.md`). A signed-out visitor picking a story gets the sign-in dialog on the
gallery with no loader and lands on the story afterwards; a direct storyline link opens the dialog by itself.
Owner-verified on dev before promotion; e2e covers the email/password path. If Google sign-in ever lands on `/`
again, check Supabase Auth → URL Configuration first: an address missing from Redirect URLs silently falls back
to the Site URL.

Still outstanding on production after that promotion:

- **115** — see its migration row for the 5-row decision it needs first.
- **`image_prompt_compiler_mode` is still `shadow`**, so every image prompt sent is the legacy one and none of
...
Before this, prod had been **at parity with dev as of 2026-08-26**, verified directly: 095 (9 Runware rows,
all disabled), 096, 097 and 098 applied on both.
...
| Image prompt compiler | `image_prompt_compiler_mode` | **`new`** — compiled prompts are sent | **`shadow`** — compiled and compared, but the **legacy prompt is what gets sent** (`assemble.shared.ts`, the `mode === 'shadow'` branch). This is why a beat's copied image prompt on production still looks like the old format: the Copy button reads `finalImagePromptText`, which in shadow mode *is* the legacy prompt. Flip at **Admin → Global Settings → Image prompt compiler** (`/admin/settings/prompt-compiler`) → "New (send compiled)" |
...
| Agentic Creator System | six `agentic_*` flags | present; **`agentic_creator_enabled` + `agentic_billing_bypass_enabled` ON** since 2026-09-07, other four off. 102–108 all applied (105 on 2026-09-06, 106/107 on 2026-09-07, 108 on 2026-09-08), 15 personas seeded (all `draft`), **two real drafts generated** and sitting at `awaiting_review` | **present and entirely off** — 102-114 and 116-118 applied 2026-09-16, 15 personas seeded (all `draft`), 0 reviewers, all six flags `false`. The code is deployed but the system is dormant; turning it on is a separate deliberate act, and `AGENTIC_SYSTEM_USER_ID` must be set on Vercel first or agent runs are denied rather than billed |
| Shared branching (continuing / forking someone else's story) | **none, deliberately (D23)** — the enforcement point is `beats` RLS, and a Postgres policy can't cheaply read `feature_flags`, so a flag here would gate the button while the database kept accepting the write | **dormant, application-level only** — Phase 10 Round 1's gates are code-complete on `feat/agentic-creator` (doorway removed, `/story/[id]` + `/explore/[id]` gated owner-or-reviewer, `continueStory` refuses a non-owner/non-reviewer before `authorize`); the database backstop, **migration 115, is applied on dev** (2026-09-12) but not on production | **application-level gates are now live** (deployed 2026-09-16 with `084c2ed`): the doorway is gone and both route layouts are owner-or-reviewer. The **database backstop is not** — 115 is still unapplied on production, so the original broader migration-003 `beats` policies still govern there and a direct `saveBeat` bypassing the UI is still permitted by the database |
...
- **RESOLVED, partially — the `beats` INSERT policy no longer lets any signed-in user write into someone
  ...
  closed** — the RLS half is migration 115, **applied on dev 2026-09-12 but still not on production** (see the
  migration table and "Shared branching" above). On production, therefore, a direct `saveBeat` invocation
  bypassing the now gated UI is still permitted by the database itself; the three application-level changes
  are what actually stop it there, not RLS.
