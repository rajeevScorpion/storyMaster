...
| 4a compiler-v2 core (pure) | — | **interrupted**: the agent hit the session limit with everything UNCOMMITTED in the working tree (capability, compile, scene-spec, relevance, assemble, admin studio, their tests, and both migration 122 files). Unreviewed and ungated. Its last finding: `redact()` in `compile.shared.ts` strips `\n` as a control character, so every section break becomes a space — pre-existing, and why compiled prompts arrive as one block; compiler-v2 must preserve newlines. Brief: capability, migration 122, scene spec 1.1, sections, budget tiers, English gate, plus humanized snake_case camera values |
