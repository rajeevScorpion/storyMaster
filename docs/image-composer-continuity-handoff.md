...
**Usage history:** 78% at ~23:10 (4a running); the window reset overnight; 16% at the 4a restart; 34% after 4b; **83% with Unit 5 in flight** (2026-09-16). Delegation stops at 90%, so nothing new was dispatched after that reading. If Unit 5's agent is lost to a limit, its work stays uncommitted in the working tree — review it against the Unit 5 brief in the briefs doc, finish it, gate it, commit.

**Still open when this was written:** Unit 6 is mostly done already (`docs/image-prompt-compiler.md`, five GOTCHAS entries, the migration ledger); what remains is the continuity/presence/reference documentation that depends on Unit 5's final shape, plus PROJECT_STATE's deferred list. After Unit 5: run the full gate suite directly (don't trust an agent report), then recommend the `--no-ff` merge into `dev`. **Length decision, resolved 2026-09-16:** the composer brevity rules were tightened (invariants may not restate style, appearance or layout; every image-facing field capped), cutting Luna 4,683 to 3,422 chars and Gemini 3,719 to 3,162 with no information lost, and migration 123 moves the target to 3,000 -> 3,800 so `over_target` flags an unusual beat again. Merged as `bc7db19`; **123 is unapplied everywhere**.
...
| 5 continuity model | `94696ca` | done; 1430 unit tests. Attribute-specific continuity (LOCKED age flips to EVOLVE on a big jump), must-not-inherit additions, prior-state note filtering, `camera_repetition`, presence-scoped CHARACTERS/absent lines, references filtered by presence, previous-storyboard reference skipped on a big beat transition unless no present character has one, `deriveOpenThreads` uses `nextBeatGoal` only, sameness wording replaced across the story, series-bible, image and regeneration prompts. Composer invariants deliberately untouched |
| 6 docs | `6febdc3`, `de04b11`, `497fba9` + this row | done bar the final summary: compiler doc rewritten for v2, six GOTCHAS entries, migration ledger updated. Remaining: PROJECT_STATE deferred list (framework phases 3–4) |
...
- **Never write a `\uXXXX` escape into source through the agent tooling.** It decodes to the real character before the file is written, so a control-character class becomes literal control bytes — including NUL, which makes git treat the file as binary and undiffable. This bit Unit 2 (worked around) and Unit 4a (`CONTROL_RE` in `compile.shared.ts`, `stripControlChars` in `scene-spec.shared.ts`, repaired during review by building both from strings: `new RegExp('[\\u0000-\\u0009\\u000B-\\u001F\\u007F]', 'g')`). The same applies to prose comments — a backslash-u sequence in a comment becomes a real newline and breaks the file.
...
- **Real beats land in tier 2, not tier 0 (live, 2026-09-16, same smoke extended through the compiler).** The composer's own plan compiled to **4,683 chars on Luna** and **3,719 on Gemini 3.8 Flash** — both English, all sections present, no Devanagari, but both over the 3,000 target after the lossless passes, so both carry `over_target`. Within policy (hard cap 5,000) yet thin: Luna is ~300 chars under the cap before 4b reserves reference lines. If that cushion matters, tighten the composer's brevity limits (description ≤ 50 words, identityAnchors ≤ 25, currentAppearance ≤ 30 are already in the template but are being exceeded) rather than compressing harder — compression past the lossless passes drops real content. Owner decision; `over_target` on every beat also makes that warning useless as a signal.
