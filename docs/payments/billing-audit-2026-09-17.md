...
| # | Finding | What goes wrong | Where | Source |
|---|---|---|---|---|
| H1 | **Payers get no notification of any kind.** Razorpay subscription notifications are off and the app has no email provider. **Stream 06:** the RBI 24-hour pre-debit notice is sent by the card issuer or bank, so this is **not** a regulatory breach. With `customer_notify: 0`, though, Razorpay also withholds its failed-payment email containing the **update-card link**. Razorpay retries daily for about 3 days, then halts, so a failing subscriber gets no chance to fix it. | Renewal charges show up only on the card statement → chargebacks, complaints. | `lib/billing/razorpay.ts:110`; `package.json` has no email provider | P-S5-2, P-S5-10, R |
...
| H9 | **Top-up coins are granted on signature alone**, without confirming capture, and orders don't request capture explicitly. **Stream 06:** the account default is auto-capture, and the old `payment_capture` order field is deprecated. A payment left `authorized` is auto-refunded after 3 days, so a signature alone isn't proof of captured funds. | If capture is manual or late, coins are granted for money never captured. | `verify/route.ts:115-132`; `lib/billing/razorpay.ts:122-137` | C-F6, R |
| H10 | **Subscription coins are granted on status `authenticated` as well as `active`.** **Stream 06:** for cards with no `start_at`, the authentication charge is the real first payment. For UPI Autopay and eNACH, Razorpay's docs also describe a ₹1–5 uncaptured token authorisation, so `authenticated` may mean **no money yet**. Unresolved from public docs; needs a sandbox trace per method. | Possibly coins before money. | `razorpay-sync.ts:188` | E-R-1 |
...
| 06 Razorpay capabilities | complete | Answered H1 (not a legal breach), H9, H10. Also: Razorpay's Invoices API can't make GST invoices; no customer self-service portal exists; domestic-card subscriptions can't switch plans via the update API; UPI Autopay mandates may cap at 10 years against the 1,200-cycle `total_count` (unconfirmed) |
