...
// purpose: the imported server actions (callTextModelOutcome, reel moods) resolve to
...
import { callTextModelOutcome } from '@/app/actions/text-model-proxy';
import { isContentBlockedError, unwrapTextOutcome } from '@/lib/ai/text-gateway/outcome.shared';
...
export async function generateStoryBeat(
  ...
  try {
    const generateAttempt = async (repairNote?: string): Promise<StoryBeat> => timeRuntimeStep(
      ...
      async () => {
        const text = unwrapTextOutcome(await callTextModelOutcome({
...
export async function composeStoryboardPlan(
  ...
  return timeRuntimeStep(
    ...
    async () => {
      ...
      let text = '';
      try {
        text = unwrapTextOutcome(await callTextModelOutcome({
          task: promptTask,
          model: isReel
